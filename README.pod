...
first command-line argument ( `$ARGV[0]` ) or, if no argument is supplied, the
current directory ( `$ENV{PWD}` ). Next, `File::Find` is used to make a list of
...
Future versions will have an option to generate a site-map ( using all .html
